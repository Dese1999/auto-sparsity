...
def get_model(args):
    ...
    if args.arch == 'resnet18':
        ...
        # فقط برای مدل‌های غیر Split_ResNet18 تغییر conv1 اعمال بشه
        if args.arch != 'Split_ResNet18' and (args.set == "CIFAR100" or args.set == 'CIFAR10'):
            args.logger.info("Modifying conv1 for CIFAR dataset: kernel_size=3")
...
def extract_new_sparse_model(cfg, net, fish, generation):
    """
    Extract sparse model and initialize learnable masks with SNIP.
    """
    net_mask_current = create_dense_mask_0(deepcopy(net), cfg.device, value=0)
    dict_FIM = {}
    with torch.no_grad():
        for (name, param), param_mask in zip(net.named_parameters(), net_mask_current.parameters()):
            if 'weight' in name and 'bn' not in name and 'downsample' not in name:
                if name not in fish:
                    print(f"Warning: No fisher_mat entry for {name}")
                    continue
                param_fish = fish[name]  # directly used dictionary
                ...
                if cfg.weight_pruning:
                    ...
                    param_mask = torch.flatten(param_mask)
                    param_mask.data[top_k_idx] = 1
                param_mask = param_mask.reshape(param.shape)  #
...
def reparameterize_non_sparse(cfg, net, net_sparse_set):
    """
    Reinitialize non-important weights based on learnable masks.
    """
    # گرفتن نام‌های لایه‌های وزن‌دار
    weight_names = [name for name, _ in net.named_parameters() if 'weight' in name and 'bn' not in name and 'downsample' not in name]
    # گرفتن نام‌های ماسک‌ها
    mask_names = [name for name, _ in net.named_parameters() if 'binary_masks' in name]
    
    if len(weight_names) != len(net_sparse_set):
        raise ValueError(f"Mismatch: {len(weight_names)} weights but {len(net_sparse_set)} masks")
    
    # Create a dictionary to access masks
    mask_dict = {name: param for name, param in zip(mask_names, net_sparse_set)}
    
    for name, param in net.named_parameters():
        if 'weight' in name and 'bn' not in name and 'downsample' not in name:
            # Finding the corresponding mask
            mask_idx = weight_names.index(name)
            mask_name = mask_names[mask_idx]
            mask_param = mask_dict[mask_name]
            
            if param.shape != mask_param.shape:
                raise ValueError(f"Shape mismatch: param {name} has shape {param.shape}, but mask {mask_name} has shape {mask_param.shape}")
            
            re_init_param = torch.randn_like(param) * 0.01
            param.data = param * binarize(mask_param) + re_init_param * (1 - binarize(mask_param))
            #print(f"Reparameterized {name} with mask {mask_name}")
...
def initialize_learnable_masks_with_snip(cfg, model, train_loader, fisher_mat=None):
    if fisher_mat is None:
        ...
        print(f"Using provided fisher_mat for initialization")


    print(f"fisher_mat keys: {list(fisher_mat.keys())}")
    print(f"fisher_mat shapes: {[(name, tensor.shape) for name, tensor in fisher_mat.items()]}")
   
    binary_mask_names = [name for name, _ in model.named_parameters() if 'binary_masks' in name]
    print(f"Binary mask names: {binary_mask_names}")
   
    weight_names = [name for name, _ in model.named_parameters() if 'weight' in name and 'bn' not in name and 'downsample' not in name]
    print(f"Weight names: {weight_names}")
   
    if len(binary_mask_names) != len(fisher_mat) or len(binary_mask_names) != len(weight_names):
        raise ValueError(f"Mismatch: {len(binary_mask_names)} binary masks, {len(fisher_mat)} fisher_mat entries, {len(weight_names)} weight names")
   
    for idx, mask_name in enumerate(binary_mask_names):
        weight_name = weight_names[idx]
        if weight_name not in fisher_mat:
            raise ValueError(f"No fisher_mat entry for {weight_name}")
       
        param = dict(model.named_parameters())[mask_name]
        param.data = (fisher_mat[weight_name] > 0).float()
        zeros = torch.sum(param == 0).item()
        total = param.numel()
        print(f"Initialized binary_mask {mask_name} (for {weight_name}): {zeros}/{total} zeros ({zeros/total*100:.2f}%)")
   
    total_params = sum(m.numel() for m in model.binary_masks)
    zero_params = sum(torch.sum(m == 0).item() for m in model.binary_masks)
    sparsity_percent = (zero_params / total_params) * 100 if total_params > 0 else 0.0
    print(f"After SNIP: Total sparsity percent: {sparsity_percent:.2f}%")
