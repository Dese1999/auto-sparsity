...
def train(train_loader, model, criterion, optimizer, epoch, cfg, writer, mask=None):
    ...
    # Review of mask retraining
    relearn_mask = False
    if cfg.mask_relearn_epochs == 0: 
        relearn_mask = True
    elif isinstance(cfg.mask_relearn_epochs, int):  
        if (epoch + 1) % cfg.mask_relearn_epochs == 0:
            relearn_mask = True
    elif isinstance(cfg.mask_relearn_epochs, list): 
        if (epoch + 1) in cfg.mask_relearn_epochs:
            relearn_mask = True

    if relearn_mask:
        model = net_utils.initialize_learnable_masks_with_snip(cfg, model, train_loader, fisher_mat=mask)
        cfg.logger.info(f"Generation {cfg.current_generation}, Epoch {epoch}: Relearned masks")
        # Record sparsity after retraining
        log_mask_stats(cfg, model, epoch, cfg.current_generation)
    ...
    # Save masks in case of retraining
    if relearn_mask and cfg.save_model:
        torch.save(
            model.binary_masks.state_dict(),
            os.path.join(cfg.exp_dir, f"mask_gen{cfg.current_generation}_epoch{epoch}.pth")
        )
